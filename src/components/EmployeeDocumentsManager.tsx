...
export default function EmployeeDocumentsManager({ employeeId, userId }: Props) {
  ...
  const fetchDocuments = async () => {
    ...
    // Also fetch legacy sick notes from time_entries (stored outside krankmeldung/ subfolder)
    const { data: legacyEntries } = await supabase
      .from("time_entries")
      .select("notizen, created_at")
      .eq("user_id", targetUserId)
      .ilike("notizen", "Krankmeldung:%");

    if (legacyEntries) {
      const existingNames = new Set(newDocs.krankmeldung.map(d => d.name));
      for (const entry of legacyEntries) {
        const noteText = entry.notizen?.replace("Krankmeldung:", "").trim() || "";
        if (noteText.includes("/")) {
          const fileName = noteText.split("/").pop() || "";
          if (fileName && !existingNames.has(fileName)) {
            existingNames.add(fileName);
            newDocs.krankmeldung.push({
              name: fileName,
  ...
  const handleUpload = async (type: DocumentType, files: FileList | null) => {
    ...
    try {
      ...
      if (userId) {
        const typeLabel = type === "lohnzettel" ? "Lohnzettel" : "Krankmeldung";
        await supabase
          .from("notifications" as any)
          .insert({
            user_id: userId,
            type: "document",
            title: `Neuer ${typeLabel} verfügbar`,
            message: `${files.length} ${typeLabel}-Dokument(e) wurden für Sie hochgeladen.`,
          });
      }
  ...
  const resolveFilePath = async (type: DocumentType, fileName: string): Promise<string> => {
    const targetUserId = userId || employeeId;
    const standardPath = `${targetUserId}/${type}/${fileName}`;
    // Try standard path first
    const { data } = await supabase.storage.from("employee-documents").list(`${targetUserId}/${type}`, { search: fileName });
    if (data && data.length > 0) return standardPath;
    // Fallback: file at root level (legacy)
    return `${targetUserId}/${fileName}`;
  };

  const handleDownload = async (type: DocumentType, fileName: string) => {
    try {
      const filePath = await resolveFilePath(type, fileName);
      const { data, error } = await supabase.storage
        .from("employee-documents")
        .download(filePath);
