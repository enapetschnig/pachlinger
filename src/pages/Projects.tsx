...
import { FolderOpen, Plus, FileText, Image, Package, Lock, Search, Upload, Camera, Trash2, ChevronDown, Pencil } from "lucide-react";
...
const Projects = () => {
  ...
  const handleUpdateProject = async () => {
    ...
    if (!editProject.name.trim() || !editProject.plz?.trim()) {
      toast({ variant: "destructive", title: "Fehler", description: "Name und PLZ sind Pflichtfelder" });
      setSavingEdit(false);
      return;
    }
    ...
    if (error) {
      toast({ variant: "destructive", title: "Fehler", description: "Projekt konnte nicht aktualisiert werden" });
    } else {
      toast({ title: "Erfolg", description: "Projekt wurde aktualisiert" });
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex gap-2">
                    ...
                    />
                    <LocationButton
                      onAddressFound={(address) => setNewProject({ ...newProject, adresse: address })}
      ...
      <Dialog open={showEditDialog} onOpenChange={(open) => {
        ...
        <DialogContent className="max-w-sm sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>Projektdaten ändern</DialogDescription>
          </DialogHeader>
          {editProject && (
            <div className="space-y-4">
              <div className="space-y-2">
              ...
              <div className="space-y-2">
                ...
                <Input
                  id="edit-plz"
                  value={editProject.plz || ''}
                  onChange={(e) => setEditProject({ ...editProject, plz: e.target.value })}
                  maxLength={5}
                />
              </div>
              <div className="space-y-2">
              ...
              <div className="space-y-2">
                ...
                <Textarea
                  ...
                  value={editProject.beschreibung || ''}
                  onChange={(e) => setEditProject({ ...editProject, beschreibung: e.target.value })}
                  className="min-h-20"
                />
              </div>
              <div className="flex gap-2">
                <Button onClick={handleUpdateProject} className="flex-1" disabled={savingEdit}>
                ...
                <Button
                  ...
                  onClick={() => {
                    setShowEditDialog(false);
                    setProjectToDelete({ id: editProject.id, name: editProject.name });
                  }}
                >
