...
export default function MyDocuments() {
  ...
  const fetchDocuments = async (
    ...
    const docs = (data || []).map((file) => ({
      name: file.name,
      path: `${uid}/${type}/${file.name}`,
      created_at: file.created_at,
    }));

    // For sick notes, also check legacy paths from time_entries
        ...
        for (const entry of legacyEntries) {
          ...
          if (noteText.includes("/")) {
            ...
            if (fileName && !existingNames.has(fileName)) {
              ...
              docs.push({
                name: fileName,
                path: noteText, // full legacy path e.g. userId/timestamp_file.jpg
  ...
  const handleUpload = async (type: "lohnzettel" | "krankmeldung", file: File | null) => {
    ...
    if (error) {
      ...
      // Notify admins about new sick note
      if (type === "krankmeldung") {
        try {
          await supabase.rpc("create_sick_note_notification" as any, {
            p_title: "Neue Krankmeldung",
            p_message: `Ein Mitarbeiter hat eine neue Krankmeldung hochgeladen: ${file.name}`,
          });
        } catch (notifError) {
  ...
  return (
    <div className="min-h-screen bg-background">
      <PageHeader title="Meine Dokumente" backPath="/" />
